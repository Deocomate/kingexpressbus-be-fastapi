"""Tour booking status transitions."""
...
from app.infrastructure.persistence.models import TourBooking


async def update_tour_booking_status(
    db: AsyncSession,
    booking_id: int,
    status: str,
    *,
    ...
    booking = (
        await db.execute(
            select(TourBooking).where(TourBooking.id == booking_id).with_for_update()
        )
    ).scalar_one_or_none()
    if booking is None:
        raise BookingError("Tour booking not found", status_code=404)
